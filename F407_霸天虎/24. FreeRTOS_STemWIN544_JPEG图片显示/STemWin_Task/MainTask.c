...
static void ShowJPEGEx(const char *sFilename)
{
	...
	GUI_JPEG_DrawEx(_GetData, &file, 40, 80);
...
static void ShowJPEG(const char *sFilename)
{
	...
	GUI_JPEG_Draw(_acbuffer, file.fsize, 40, 80);
...
static WM_HMEM LoadJPEG_UsingMEMDEV(const char *sFilename)
{
	...
	/* 创建内存设备 */
	hJPEG = GUI_MEMDEV_CreateEx(40, 80,                /* 起始坐标 */
  ...
  /* 加载JPEG图片数据到内存设备 */
    ...
    switch(i)
    {
			case 1:
				...
				t1 = GUI_GetTime();
        GUI_DispStringHCenterAt("GUI_JPEG_DrawEx()", 240, 15);
        printf("\r\n直接从外部存储器绘制JPEG：%dms\r\n",t1 - t0);
				break;
			case 2:
				t0 = GUI_GetTime();
				...
				t1 = GUI_GetTime();
        GUI_DispStringHCenterAt("GUI_JPEG_Draw()", 240, 15);
        printf("加载JPEG到内存中并绘制：%dms\r\n",t1 - t0);
				break;
      case 3:
        t0 = GUI_GetTime();
        /* 从内存设备写入LCD */
        GUI_MEMDEV_CopyToLCDAt(hJPEG, 40, 80);
        t1 = GUI_GetTime();
        GUI_DispStringHCenterAt("USE MEMDEV", 240, 15);
