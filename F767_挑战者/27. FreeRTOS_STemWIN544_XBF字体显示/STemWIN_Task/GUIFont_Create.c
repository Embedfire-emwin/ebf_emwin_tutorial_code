...
/* 字库存储路径 */
	...
	static const char FONT_XINSONGTI_18_ADDR[] = 	 "0:/Font/xinsongti18.xbf";
	static const char FONT_SIYUANHEITI_36_ADDR[] = "0:/Font/siyuanheiti36_2bpp.xbf";
...
int GetResOffset(const char *res_name)
{
	...
	for(i=0;i<CATALOG_SIZE;i+=sizeof(CatalogTypeDef))
	{
		...
		BSP_QSPI_Read((uint8_t*)&Catalog,RESOURCE_BASE_ADDR+i,sizeof(CatalogTypeDef));
...
static int _cb_FONT_XBF_GetData(U32 Offset, U16 NumBytes, void * pVoid, void * pBuffer)
{
	...
	/* 读取内容 */
	BSP_QSPI_Read(pBuffer,FONT_BASE_ADDR+Offset,NumBytes);
