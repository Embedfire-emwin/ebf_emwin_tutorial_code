...
void Burn_Catalog(void)
{
  ...
  for(i=0;1;i++)
  {
    ...
    state = BSP_QSPI_FastRead(flash_buf,RESOURCE_BASE_ADDR + sizeof(dir)*i,sizeof(dir)); 
    
    Buffercmp((uint8_t*)&dir, flash_buf, sizeof(dir));
    ...
    LED_BLUE;
     ...
     result = f_open(&file_temp,full_file_name,FA_OPEN_EXISTING | FA_READ);
      ...
      while(result == FR_OK) 
      {
        ...
        state = BSP_QSPI_FastRead(flash_buf,write_addr,bw);  

        state = Buffercmp(tempbuf, flash_buf, bw);
        if(state != QSPI_OK)
        {
          BURN_ERROR("Burn_Content QSPI_Write ERROR");
          BURN_ERROR("loop=%d bw =%d addr = %d",loop,bw,write_addr);
          BURN_ERROR("tempbuf");
    ...
    BURN_INFO("内容大小：%d ",dir.size);
      ...
      while(result == FR_OK) 
      {
        ...
        for(j=0;j<bw;j++)
        {
          ...
          {
            BURN_ERROR("数据校验失败！");
            BURN_ERROR("read_addr=%d ",read_addr-bw);
            BURN_ERROR("loop=%d ",loop);
            BURN_ERROR("j=%d ",j);
            BURN_ERROR("tempbuf");
            BURN_DEBUG_ARRAY(tempbuf,bw);
